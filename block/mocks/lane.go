...
// GetRatio provides a mock function with given fields: ctx
func (_m *Lane) GetRatio(ctx types.Context) (math.LegacyDec, error) {
	ret := _m.Called(ctx)

	if len(ret) == 0 {
		panic("no return value specified for GetRatio")
	}

	var r0 math.LegacyDec
	var r1 error
	if rf, ok := ret.Get(0).(func(types.Context) (math.LegacyDec, error)); ok {
		return rf(ctx)
	}
