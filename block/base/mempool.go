...
type (
	...
	Mempool[C comparable] struct {
		...
		// laneName defines the name of the lane.
		laneName string

		// laneKeeper defines the keeper that is responsible for storing the lane parameters.
...
func (cm *Mempool[C]) AssertLaneLimits(ctx sdk.Context, tx sdk.Tx) error {
	...
	ratio, err := cm.laneKeeper.Ratio(ctx, cm.laneName)
	if err != nil {
		return fmt.Errorf("failed to get lane ratio: %w", err)
	}
